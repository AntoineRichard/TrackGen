...
A useful reporting design is to distinguish two fields rather than force this
...
simulator scene. Equivalent schemas are acceptable when they preserve this distinction
and the raw-to-derived lineage. A derived tag should be treated as primary only when it
carries the definition needed to reconstruct the relevant course object. The families
below organize analytic differences; they do not assign a rank.
...
Parametric curves can encode continuity and differentiability when their
parameterization and composite joins enforce those properties. A Bezier control-point
sequence can define a road curve directly; moving one control point changes a global
curve and makes curvature, self-intersection, and clearance testable in the same
parameter domain. Kl\"uck \emph{et al.} use precisely this relationship between
control points, Bezier roads, and candidate validity checks
...
A sampled centerline $c(s)$ with left and right widths $w_L(s),w_R(s)$ is compact
when order, arc-length convention, orientation, and frame are declared. It retains
ordered samples and a progress coordinate. Tangents and curvature are derived: they
require declared interpolation or differencing and incur discretization loss or error.
Boundaries still require stated surface assumptions. The TUM race-track database
releases centerline coordinates with left and right widths, while treating optimized
racing lines as a distinct derived product
...
Buoy and marker courses expose the same issue in a more concrete form. Bingham
\emph{et al.} present VRX as a Gazebo-based development and testing environment for
unmanned surface vessels, illustrated through its challenge implementation
~\cite{Bingham2019TowardMaritime}. That simulator realization does not by itself
establish an authoritative ordered passage relation; inferring a gate sequence from
visual marker placement would silently add task semantics. Gate poses, waypoint
sequences, and buoy placements should therefore carry an explicit order, partial
order, or transition graph whenever completion depends on it.
...
\begin{table}[t]
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.22\linewidth}Y Y@{}}
    ...
    \midrule
    Segment grammar / topology graph & Element types and connectivity; grammar steps, ports, or provenance only when encoded & Metric geometry, widths, surface, route semantics, and unencoded order or provenance \\
    Parametric or sampled centerline + widths & Order and width profile; curve tangents if parametric, centerline samples if discrete & Surface, obstacles, junction choices; sampled tangent or curvature without declared interpolation or differencing \\
    Boundary pair / lanelet & Traversable extent and asymmetric bounds; adjacency only through explicit lanelet relations & Canonical progress coordinate, route order, elevation and rule semantics unless supplied \\
