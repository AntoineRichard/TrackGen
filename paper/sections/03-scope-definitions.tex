...
The central object of this survey is a \emph{course}: a spatial task specification
that declares task-defining elements, a traversal relation over those elements, and
terminal or completion conditions. The traversal relation may be a total sequence,
a partial order, a directed successor graph, or a task- or policy-conditioned
admissible transition relation. Accordingly, ``ordered'' means that precedence or
successor semantics are explicit; it does not require every pair of elements to be
totally ordered. A course is not merely a collection of geometric primitives,
obstacles, or visual assets. A ground course can be a closed or open corridor with
an associated route or successor relation, an aerial course a set of gate poses with
declared precedence, and a maritime course a set of waypoints, marks, or buoy-defined
gates with declared admissible transitions. These semantics make geometry
consequential to the task: traversing a gate in a disallowed direction or visiting a
required marker before its predecessor need not count as progress. Ground racing
environments, drone-racing tracks with gates, and sailing race rules illustrate
these different realizations of the same abstraction
\cite{KlimovNodateCarRacing,Kaufmann2018DeepDrone,WRSCIRSCOrganizingCommittee2019WorldRobotic}.
...
or permitted passage direction. The recommended benchmark interface by which those
attributes are serialized and tested is specified separately in
\cref{sec:benchmark-protocol}; here the purpose is to establish distinctions that
prevent incompatible objects from being treated as interchangeable evidence.
...
The \emph{course object} is the abstract task-defining spatial entity. It comprises
task-defining spatial elements, their declared traversal relation, and terminal or
completion conditions; examples include a traversable corridor with a route relation,
a gate chain, a waypoint relation, or a buoy course. It specifies what is to be
traversed, not how it is stored or rendered. For a road network, a graph alone is
insufficient unless the relevant route or admissible successor relation and terminal
conditions are stated; for an aerial gate course, positions alone are insufficient
when gate orientation, crossing direction, or precedence matters.
...
Likewise, official maritime rules can define gate or mark-passage requirements,
completion, and scoring without supplying an artifact-synthesis procedure
\cite{RoboNation20222022Maritime,WRSCIRSCOrganizingCommittee2019WorldRobotic}.
