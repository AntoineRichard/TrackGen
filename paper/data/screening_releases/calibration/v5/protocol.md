# Duplicate full-text screening codebook v3

This codebook prospectively governs eligibility screening for the course-generation
survey. It applies to the complete frozen population of 202 candidate reports. Each
report receives two independent, blind, full-text eligibility ratings under one final
protocol and coordinator snapshot version. Calibration, main screening, adjudication,
work-level deduplication, and author verification are separate controlled stages.
...
| `boundary transfer` | A boundary transfer MUST originate in fixed-course racing or control work, be explicitly selected by this survey as a transferable requirement, metric, dataset property, or reporting practice, and retain a precise source locator. It MUST NOT imply that the source contributes to course generation. |
| `material evidence` | Material evidence MUST be content inspected directly in the report being screened or in its authoritative companion artifact, specific enough to decide eligibility and support the resulting survey claim. Primary reports or authoritative artifacts are required for technical contributions; a directly inspected survey or systematic review is permitted as secondary evidence only for `include-4`. Evidence MUST identify the source-specific fact, a precise locator, access mode, version, retrieval date, and required provenance. Topic similarity, a title, or an unsupported abstract inference is not material evidence. |
...
assigned batches. Before both ratings are locked, neither reviewer may see the other
reviewer's status, criterion, evidence, locator, exclusion reason, notes, work in
progress, execution trace, or retrieval cache. Reviewer-facing packets MUST omit prior
screening decisions, exclusion reasons, unresolved-conflict flags, discovery provenance
not required for retrieval, citation keys, and any other field that reveals a prior
eligibility decision. Bibliographic metadata and the source itself remain available
because they are needed for retrieval and verification.
...
and requires isolated execution contexts and retrieval.
...
| `exclusion_reason` | `NR` for `included` or `boundary`; a substantive source-specific reason for `excluded`. |
| `notes` | Concise supplementary information, including an adjacent-domain mapping when applicable, or exactly `NR`. |
...
unlisted value, spelling variant, capitalization variant, or legacy code.

### Screening statuses

| Value | Normative meaning |
| --- | --- |
| `included` | The report directly satisfies at least one source-native inclusion criterion. |
| `boundary` | No inclusion criterion applies, but fixed-course racing or control work supplies a named survey-level boundary transfer. |
| `excluded` | Neither inclusion nor boundary applies, or accessible evidence is insufficient to support a survey claim. |

### Inclusion criteria

When several inclusion criteria apply, record the first applicable criterion in the
order shown and describe additional applicability in `notes`.
...
| `include-3` | Makes a source-native contribution that defines or validates a metric, feasibility test, or dynamics-grounded model by applying it to variation across generated or parameterized courses. |
| `include-4` | Is a survey or systematic review directly needed to establish, organize, or substantiate the course-generation survey gap. |

### Inclusion-boundary precedence clarification

When one source both performs a qualifying `include-1` operation and defines an
`include-2` representation, interface, dataset, benchmark, course set, simulator
contract, or interchange artifact, reviewers MUST record `include-1`. The additional
`include-2`-type contribution MUST be recorded in `notes` but MUST NOT replace the
primary criterion. `include-2` is selected only when no qualifying `include-1`
operation is established. Merely loading, referencing,
displaying, or controlling on supplied fixed geometry is not `include-1`.

A directly inspected source-native script, implementation, algorithm, or specification
that emits, samples, places, connects, or serializes explicit course coordinates,
corridors, routes, waypoints, gates, or buoys satisfies `include-1`, even when a
downstream benchmark later uses one generated realization as a fixed course. A source
that normatively defines a reusable course representation, interchange format, design
interface, simulator course interface, dataset, or competition course set for generated
or parameterized courses defines an `include-2`-type contribution.

Merely loading, controlling on, evaluating on, or describing a fixed course does not
satisfy inclusion. Such work is `boundary` only when the inspected source provides a
specific citable requirement, metric, dataset property, or reporting practice that this
survey explicitly transfers. A generic laboratory, project, promotional, or topic page
does not become `boundary` from broad relevance or possible future reuse; absent
material technical evidence for a named transfer, it is `excluded`.

### Boundary criterion

| Value | Normative meaning |
| --- | --- |
| `boundary` | No inclusion rule applies, and fixed-course work supplies material evidence for a specifically named survey-level boundary transfer: a requirement, metric, dataset property, or reporting practice. Fixed-course work classified this way MUST NOT be treated as a source-native contribution. |

### Exclusion criteria

| Value | Normative meaning |
| --- | --- |
| `exclude-fixed-racing-line` | Only plans, optimizes, predicts, or controls a racing line in a fixed corridor and supplies no named boundary transfer. |
| `exclude-appearance-dynamics` | Only randomizes appearance, sensing, vehicle parameters, disturbances, or dynamics without generating, selecting, repairing, validating, or serializing course geometry. |
| `exclude-traffic-only` | Only generates traffic participants, behavior, scenarios, or interactions on fixed roads without contributing course geometry or a qualifying boundary transfer. |
| `exclude-insufficient-detail` | Mentions a course, road, track, gate sequence, generator, benchmark, or metric but accessible evidence is too incomplete to support a survey claim. |
| `exclude-out-of-scope` | Has no material contribution to course generation, representation, characterization, feasibility, benchmarking, simulator interchange, or the survey-gap literature. |

### Access statuses

| Value | Normative meaning |
| --- | --- |
| `full_text` | The complete primary report was inspected. |
| `full_text_and_supplement` | The complete primary report and relevant supplementary material or repository documentation were inspected. |
| `official_documentation` | Official, version-identifiable documentation was inspected for a software, dataset, benchmark, standard, or competition artifact. |
| `abstract_only` | Exhaustive retrieval failed and only an abstract or similarly incomplete bibliographic record was available. |

## Normative decision procedure

Inclusion has precedence over boundary and exclusion. A report is included only when
material evidence establishes a source-native contribution under the first applicable
inclusion criterion. The source itself MUST connect `include-2` or
`include-3` to generated or parameterized courses; intended reuse by this survey
is insufficient. `include-1` may also apply through a named transferable adjacent
domain, but the source-native technical mapping MUST be recorded.

Boundary MAY be considered only when no inclusion criterion applies. Boundary is a
survey-level transfer from fixed-course work and MUST NOT be used to relabel that
source as a generated-course contribution. A boundary decision MUST name the exact
requirement, metric, dataset property, or reporting practice transferred by the survey
and cite material technical evidence for it. Generic institutional relevance,
high-level project descriptions, or possible future reuse are insufficient.

1. **Establish access.** Retrieve and inspect the strongest available eligible source,
   assign one access status, and record canonical source URLs, version provenance, and
   precise locators.
2. **Apply inclusion.** Evaluate `include-1` through `include-4` in order.
   Stop at the first applicable criterion and assign `included`; record other
   applicable criteria only in `notes`.
3. **Apply boundary.** Only when no inclusion criterion applies, determine whether
   fixed-course racing or control work supports a named survey-level boundary transfer.
   Uncertainty, possible future reuse, or general topical similarity MUST NOT produce a
   boundary decision.
4. **Apply exclusion.** If neither inclusion nor boundary applies, assign `excluded`
   with exactly one controlled primary exclusion criterion and a source-specific reason.
   Choose the criterion that most directly explains why the report cannot support this
   survey; do not use a generic reason when a specific rule applies.
5. **Validate the result.** Confirm status-criterion pairing, access sufficiency,
   source and provenance fields, locator precision, reason requirements, assignment
   binding, and absence of blank fields before locking the rating.

### Status and criterion pairing

| screening_status | Allowed criterion | exclusion_reason |
| --- | --- | --- |
| `included` | Exactly one of `include-1` through `include-4` | `NR` |
| `boundary` | `boundary` | `NR` |
| `excluded` | Exactly one controlled exclusion criterion | A substantive, source-specific reason |

## Access and evidence rules

Before assigning `abstract_only`, the reviewer MUST make and document an
...
exists. Attempted locations and outcomes belong in `source_urls` and `notes`;
inaccessible or unsafe URLs MUST NOT be invented.

`included` and `boundary` MUST NOT use `abstract_only`; they require
`full_text`, `full_text_and_supplement`, or
`official_documentation`. If only an abstract remains available and it cannot
support a survey claim, the result MUST be `excluded` with criterion
`exclude-insufficient-detail`. An abstract may justify another exclusion code only
when it directly and unambiguously establishes that code.
...
publishes no version identifier, record `unversioned@YYYY-MM-DD` using the retrieval
date; this declares the limitation rather than inventing a version.

An `included` or `boundary` rating based on
`official_documentation` MUST record either a version-pinned
`evidence_archive_url` or a lowercase 64-hex `evidence_sha256`. Recording both
is preferred. A branch URL, mutable latest-documentation URL, or unpinned repository
page is not version-pinned. `evidence_archive_url` MAY be `NR` only when no
version-pinned archive was inspected. `evidence_sha256` MAY be `NR` only
when the exact inspected bytes were not lawfully or technically obtainable. If both
are `NR`, official documentation cannot support `included` or `boundary`.

For `excluded`, `exclusion_reason` MUST state the source-specific fact that
activates the selected exclusion criterion; merely repeating the criterion label is
invalid. For `included` and `boundary`, `exclusion_reason` MUST be
exactly `NR`. `notes` MAY be `NR`.
`resolved_conflict_ids` MAY be `NR` only in an adjudication with no conflict
to resolve. Only `evidence_archive_url`, `evidence_sha256`,
`exclusion_reason`, `notes`, and `resolved_conflict_ids` may contain
`NR` under their field-specific rules. No field may be blank.
...
| A2 | Any criterion disagreement, including different inclusion criteria or different exclusion criteria, even when statuses agree. |
...
```json
{"comparison_analysis":"<candidate-specific comparative analysis>","controlling_rules":["A1","A2","A3","A4"],"deciding_fact":{"kind":"transfer_source_fact","text":"<source-specific fact>"},"deciding_locator":"<complete screening_locator>","final_decision":{"criterion":"<criterion>","screening_status":"<screening_status>"},"raw_exclusion_reasons":[{"assignment_id":"<assignment_id_0>","reason":"<complete reason 0>"},{"assignment_id":"<assignment_id_1>","reason":"<complete reason 1>"}],"raw_ratings":[{"assignment_id":"<assignment_id_0>","criterion":"<criterion_0>","screening_status":"<screening_status_0>"},{"assignment_id":"<assignment_id_1>","criterion":"<criterion_1>","screening_status":"<screening_status_1>"}],"resolved_conflicts":[{"conflict_id":"<conflict_id>","field":"<field>","value_a":"<value_a>","value_b":"<value_b>"}],"schema_version":"1","source_url":"<one complete canonical source URL>"}
```
...
- For an excluded decision, `deciding_fact` MUST have
  ...
  `exclusion_reason`. For an included or boundary decision, it MUST have
  `kind=transfer_source_fact` and a substantive non-`NR` `text` of at least
  48 characters and eight words that states the deciding source or transfer fact.
...
Included and boundary adjudications remain subject to the full-text and official-
...
A previously keyless included or boundary candidate MAY be activated only through this
...
Every final `included` or `boundary` candidate MUST resolve to one active key in this
ledger. The final projected `candidates.csv` exposes that key for included and boundary
...
| Nominal Gwet AC1 | With `K=3` and `p_k=n_k/(2N)`, use `P_e=sum_k p_k(1-p_k)/(K-1)` and `AC1=(P_o-P_e)/(1-P_e)`; `1-P_e=0` is `not_estimable`. |
...
table so that equal statuses with unequal inclusion or exclusion criteria remain
visible. Missing, invalid, or duplicate ratings are hard errors rather than omissions
from denominators.
...
roles. They MAY share only the same frozen protocol and independently retrievable
public sources. When a provider does not expose retrieval-cache isolation, the frozen
profile and execution register MUST record that limitation and the paper MUST disclose
it; they MUST NOT claim technical cache isolation. No automated reviewer may be resumed
from another reviewer's context or receive a summary of another review before both
results lock.
...
retrieval provenance, inclusion or exclusion rationale, adjudication outcome, and
