# Duplicate full-text screening codebook v4
...
A report is included when material evidence establishes at least one source-native
eligibility rule. The rules are disjunctive alternatives for establishing relevance,
not competing criterion values.
...
The source itself MUST establish at least one of these source-native eligibility rules:

1. It operates on explicit course geometry or a course distribution for racing robots
   or a named transferable adjacent domain.
2. It defines a representation, design or simulator interface, dataset, benchmark,
   competition course set, or interchange artifact specifically for generated or
   parameterized courses.
3. It defines or validates a metric, feasibility test, or dynamics-grounded model by
   applying it to variation across generated or parameterized courses.
4. It is a survey or systematic review directly needed to establish, organize, or
   substantiate the course-generation survey gap.

A source satisfying several rules remains one `included`,`include-relevant` screening
result. Reviewers MUST NOT choose or rank a primary contribution subtype. Contribution
characterization occurs in downstream evidence extraction, where existing multi-valued
evidence fields record every supported contribution type. The screening `notes` field
MUST NOT substitute for downstream evidence extraction.

### Eligibility/boundary clarification
...
corridors, routes, waypoints, gates, or buoys satisfies the course-operations
eligibility rule, even when a downstream benchmark later uses one generated
...
dataset, or competition course set for generated or parameterized courses satisfies
the generated-course artifacts or interfaces eligibility rule.
...
material evidence establishes at least one source-native eligibility rule. The source
itself MUST establish at least one source-native eligibility rule; intended reuse by
this survey is insufficient. The course-operations rule may also apply through a named
transferable adjacent domain, but the source-native technical mapping MUST be recorded.
...
1. **Establish access.** Retrieve and inspect the strongest available eligible source,
   ...
   precise locators.
2. **Apply inclusion.** Determine whether material evidence establishes at least one
   source-native eligibility rule. If it does, assign
   `included`,`include-relevant`. Reviewers MUST NOT choose or rank a primary
   contribution subtype; contribution characterization occurs in downstream evidence
   extraction.
...
retrieval provenance, eligibility, boundary, or exclusion rationale, adjudication outcome, and
