# Duplicate full-text screening codebook v7
...
Final prospective v7 screening statuses are exactly `included` and `excluded`.
`boundary` is historical terminology only and MUST NOT be assigned as a v7 result,
...
`boundary` MUST NOT be assigned as a v7 result or criterion. Historical boundary
...
Before any v7 coordinator freeze or reviewer launch, the evidence inventory MUST be
complete and each duplicate pair's common evidence packet MUST be versioned and
digest-bound. V7 requires a fresh stable-30 calibration conducted in six blind reviewer
contexts and producing 60 valid ratings. Release requires agreement >= 0.80 and no
systematic ambiguity. V7 reviewers MUST NOT receive v3-v6 ratings or disagreements.
No v7 main release may be created until this gate passes.
