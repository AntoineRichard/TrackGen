...
**Source/access.** Provided local primary PDF `paper/data/source_archive/provided/ICSE-NIER66352.2025.00021.pdf` (SHA-256 `8b5119b6edc8bce2bfb360a279a33b20e47fbca9d803a61a2e9cb18890cd020a`; Abstract; Sections II-III and V; Figures 2-4; Direct). The paper cites an uninspected replication package.

**Values.** Domain: ground autonomous-vehicle testing. Vehicle: autonomous vehicle; the evaluation test subject is BeamNG.AI. Course object: open road corridor. Representation family: segment grammar and sampled centerline; ChatGPT returns a semi-structured JSON description containing a start point, theta, and ordered segments with length, direction, incline, and turn degrees, which ROAD GPT transforms into 3D road points using straight lines/circular arcs and cubic-spline z interpolation. Generator family: learned-generative plus constructive; ROAD GPT uses ChatGPT to translate natural-language user requirements and deterministically constructs the road geometry. Generation role: geometry synthesis for focused AV-test scenarios. Validity strategy: rejection; roads outside map boundaries, self-intersecting roads, overly sharp turns, and excessive slopes are reported as errors and discarded. Reported geometry/difficulty/diversity metrics: segment geometry and elevation parameters; prompt incline, turn count/type/sequence, and prompt complexity; user pairwise diversity judgments including turn sequence and road length. Training/evaluation distribution counts: no training distribution reported; 300 roads generated (100 for each P1/P2/P3), with 182 valid (60%): P1 88 valid/1 exposing misbehavior, P2 57/2, and P3 37/5. The evaluation covers validity, exposure of ego-vehicle misbehavior, user-description match, and diversity. Simulator/export format: BeamNG.tech renders Meshroad and Decalroad objects and positions the ego vehicle; the intermediate description is semi-structured JSON, with no external interchange format stated. Code/asset status: NR; the paper cites an uninspected replication package.
