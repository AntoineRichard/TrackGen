...
def test_hull_end_to_end(dev):
    ...
    reasonable valid yield. (Observed CPU yield ~0.9 at E=64; we require >= 0.5 as a robust
    floor that still proves the method produces usable tracks, never gating diversity.)"""
    E, N_max = 64, 256
    cfg = TrackGenConfig(
    ...
    # Reasonable valid yield (states the floor; the method should clear half).
    yield_frac = float(valid.float().mean())
    assert yield_frac >= 0.5, f"hull valid yield too low: {yield_frac}"
