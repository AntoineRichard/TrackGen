...
def test_controlled_vocabularies_are_exact_and_closed() -> None:
    ...
    vocabularies = (
        ...
        ("Boundary criterion", BOUNDARY_CRITERIA),
...
def test_inclusion_is_source_native_and_boundary_is_survey_level_transfer() -> None:
    text = _protocol()
    criteria = {
    ...
    assert "fixed-course work" in boundary
    assert "MUST NOT be treated as a source-native contribution" in boundary
...
def test_decision_procedure_has_explicit_precedence_and_status_pairing() -> None:
    ...
    assert "Inclusion has precedence over boundary and exclusion." in normalized
    assert "Boundary MAY be considered only when no inclusion criterion applies." in normalized
...
def test_access_gate_and_evidence_provenance_are_executable() -> None:
    ...
    assert (
        "included and boundary MUST NOT use abstract_only; they require full_text, "
    ...
    assert (
        "An included or boundary rating based on official_documentation MUST record "
...
def test_reviewer_prompt_template_is_exact_and_auditable() -> None:
    ...
    assert (
        "Retrieve and inspect direct material evidence from independently retrieved "
        "public sources." in prompt
    )
    assert "Attempt exhaustive retrieval before using `abstract_only`." in prompt
...
def test_structured_adjudication_rationale_bindings_and_semantic_gate_are_exact() -> None:
    ...
    assert "kind=transfer_source_fact" in section
...
def test_citation_key_activation_uses_one_audited_append_only_ledger() -> None:
    ...
    normalized = _normalized(section)
    assert (
        "A previously keyless included or boundary candidate MAY be activated only "
