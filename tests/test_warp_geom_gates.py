...
from tests._warp_compare import self_intersections, thickness
...
def test_self_intersections_matches(dev):
    ...
    got=self_intersections(poly); ref=geometry.self_intersections(poly)
...
def test_thickness_matches(dev):
    ...
    assert torch.allclose(thickness(c,band), geometry.thickness(c,band), atol=1e-4)

@pytest.mark.parametrize("dev", DEVS)
def test_thickness_circle(dev):
    c=_circle(400,2.0,dev); band=torch.tensor([400//2-2],device=dev)
    assert torch.allclose(thickness(c,band), torch.tensor([2.0],device=dev), atol=2e-2)
