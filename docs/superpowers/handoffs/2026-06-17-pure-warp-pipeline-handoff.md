...
**Remaining: Task 15 (facade integration + remove the torch ORACLE modules).** Open scope decisions (ask owner): Fourier generator (not ported to Warp — drop, or keep as a torch path?); how aggressively to delete geometry/inflation/generators/relaxation-torch-backends; test-migration strategy (the per-kernel oracle tests import geometry/inflation/generators as the oracle — removing the oracle means those `== oracle` assertions must migrate to property/self-consistency checks).
