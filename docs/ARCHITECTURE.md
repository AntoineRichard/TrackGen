...
#### `generator="voronoi"` — fixed-site graph-cycle family

The Voronoi generator is the runtime-safe distillation of the Voronoi/random-geometric
spike. It keeps the useful site-cloud and angular-anchor idea, but avoids exact
Voronoi/Delaunay ridge traversal because dynamic cycle extraction and planar face walking do
not fit the fixed-shape CUDA-graph contract.

1. **Site field.** `_sample_sites_k` draws `S=voronoi_num_sites` points using one of the
   bounded `voronoi_site_layout` modes: `ring`, `void_ring`, `clustered`, or `mixed`.
   The default `void_ring` leaves a center void and biases sites into a usable annulus.
2. **Anchor selection.** `_select_anchor_sites_k` draws `K=voronoi_control_points` angular
   target sectors, jitters each sector by `voronoi_angular_jitter`, modulates target radius
   by `voronoi_radial_variation`, and snaps each target to the nearest unused site under a
   fixed all-sites scan.
3. **Smoothing.** `_chaikin_once_k` rounds the selected cycle once, then
   `_catmull_rom_dense_k` evaluates a closed Catmull-Rom loop over the augmented anchors.
4. **Fallback and resample.** The smooth loop is arc-resampled to `num_points` and checked
   for self-intersections. Crossing envs are overwritten by the selected anchor polygon,
   then normalized to the same target extent as the other first-stage families.
5. **Generation flag.** The generator writes `out_valid_wp=1` for every env; final quality
   is still decided by the shared post-relax validity gate.

The knobs are `voronoi_num_sites`, `voronoi_site_layout`, `voronoi_control_points`,
