...
- **Raise ``relax_iters`` first.** The default of 150 sweeps is calibrated for
  approximately 99.9 % yield at the library defaults. If yield is low, the most
  conservative fix is to increase ``relax_iters``; this extends solve time but
  does not restrict shapes.
...
``relax_iters`` (default 150) is the number of Jacobi sweeps. Each sweep
...
A 15 % margin leaves room for the downstream thickness validity gate. Increase
it if marginal-validity tracks are appearing (the gate checks that minimum
thickness is at least ``(1 - relax_tol) * half_width``). Decreasing it tightens
the packing but may produce more near-threshold invalids.
...
road trying to push adjacent beads apart.
...
``relax_iters=150`` CUDA graph benchmark:
